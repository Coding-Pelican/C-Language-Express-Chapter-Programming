...
typedef enum _StatusOfReservation {
    Open = 0,
    Closed,
    Selected
} StatusOfReservation;
...
static int seats[SIZE_OF_SEATING_ARR] = { Open, Closed, Closed, Open, };
...
void display_seat_check_message(){
	puts("Would you like to reserve this seat? (y or n)");
}
void display_seat_selection_input_message(){
	printf("Please select a open seat (1-%d)\n", SIZE_OF_SEATING_ARR);
}
void select_seat(TypeOfReservation type){
	int selection;
	int is_checked;
